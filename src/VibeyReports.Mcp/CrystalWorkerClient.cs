...
public sealed class CrystalWorkerClient
{
    ...
    // F6 (final review): ReportTools has no catch-all of its own, so any exception that escapes
    // InvokeAsync surfaces to the model as an SDK-level tool exception instead of the
    // {"ok":false,"error":...} shape every tool description promises. Three concrete gaps this
    // closes: Process.Start can throw (e.g. Win32Exception if the worker exe is deleted or blocked
    // after the startup probe); the "could not start" null-process case previously threw directly
    // rather than returning a Failure; and the stdin write ran before the timeout was consulted and
    // was not passed the cancellation token, so a worker that hangs before draining stdin (a plan
    // larger than the pipe's 64KB buffer) blocked that write forever with the timeout never
    // engaging. The single outer catch below covers all three; OperationCanceledException is
    // deliberately excluded so a genuine external cancellation still propagates as a cancellation
    // rather than being reported as a tool failure (external cancellation being reported as a
    // *timeout* by the inner catch below is a separate, deliberately deferred item).
    private async Task<WorkerResponse> InvokeAsync(WorkerRequest request, CancellationToken ct)
    {
        try
        {
            ...
            try
            {
                await process.StandardInput.WriteAsync(
                    JsonSerializer.Serialize(request, VibeyJson.Options).AsMemory(), timeoutCts.Token);
        ...
        {
            return WorkerResponse.Failure($"Could not run the Crystal worker: {ex.Message}");
        }
    }
